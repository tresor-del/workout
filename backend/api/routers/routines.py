...
def create_routines(db: db_dependency, user: user_dependency, routine: RoutineCreate):
    ...
    db_routines = db.query(Routine).option(joinedload(Routine.workouts)).filter(Routine.id == db_routine.id).first()
    return db_routines


@router.delete("/")
def delete_routine(db: db_dependency, user: user_dependency, routine_id: int):
    db_routine = db.query(Routine).filter(Routine.id == routine_id)
    if db_routine:
